...
def main():
    ...
    # Selecionar operação específica
    try:
    ...
    if args.modo == "matplot":
        visualizar_matplot(mol, mol_transformada, f"Operação: {desc}", destaque=vetor_destaque)
    elif args.modo == "pyvista":
        visualizar_pyvista(mol, mol_transformada, f"Operação: {desc}", destaque=vetor_destaque)
    else:
